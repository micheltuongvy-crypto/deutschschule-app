...
function LandingNav({ lang, setLang }: { lang: Lang; setLang: (l: Lang) => void }) {
  ...
  const navLinks = [
    { href: "/kurse", label: lang === "vi" ? "Khóa học" : "Kurse" },
    { href: "/vokabeln/a1", label: lang === "vi" ? "Từ vựng" : "Vokabeln" },
    { href: "/grammatik/a1", label: lang === "vi" ? "Ngữ pháp" : "Grammatik" },
    { href: "/uebungen/a1", label: lang === "vi" ? "Bài tập" : "Übungen" },
  ];
    ...
    <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-gray-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
        ...
        <div className="hidden sm:flex items-center gap-4 text-sm font-medium">
          {navLinks.map((link) => (
            <Link key={link.href} href={link.href} className="px-3 py-2 rounded-lg hover:bg-gray-100 text-gray-700">
              {link.label}
            </Link>
          ))}
